  ...
  return (
    <div className="space-y-2">
      ...
      <div className="flex items-center gap-3 py-1.5">
        ...
        <div className="flex-1 h-8 bg-muted/20 rounded-lg relative overflow-hidden border border-chrome/10">
          <motion.div
            className="absolute left-0 top-0 h-full rounded-lg"
            style={{
              background: 'linear-gradient(90deg, hsla(220, 10%, 72%, 0.15), hsla(220, 10%, 72%, 0.08))',
            }}
            initial={{ width: 0 }}
      ...
      {blocks.map((block, i) => {
        ...
        const barBg = isSupport
          ? 'linear-gradient(90deg, hsla(43, 96%, 56%, 0.5), hsla(43, 96%, 56%, 0.25))'
          : isContradict
          ? 'linear-gradient(90deg, hsla(0, 72%, 55%, 0.5), hsla(0, 72%, 55%, 0.25))'
          : 'linear-gradient(90deg, hsla(220, 10%, 50%, 0.4), hsla(220, 10%, 50%, 0.2))';
        ...
        return (
          ...
          >
            <div className="flex items-center gap-3 py-1.5">
              ...
              <div className="flex-1 h-8 bg-muted/10 rounded-lg relative overflow-hidden border border-chrome/5">
                <motion.div
                  ...
                  style={{
                    ...
                    background: barBg,
                    boxShadow: isSupport
                      ? '0 0 12px -4px hsla(43, 96%, 56%, 0.3)'
                      : isContradict
                      ? '0 0 12px -4px hsla(0, 72%, 55%, 0.3)'
                      : 'none',
                  }}
                  initial={{ width: 0 }}
                ...
                />
                <div className="absolute left-1/2 top-0 h-full w-px bg-chrome/10" />
              </div>
            ...
            {/* Hover detail */}
            <div className="hidden group-hover:block ml-[9.5rem] mb-2 glass-chrome rounded-lg p-3 text-xs space-y-1 border-gold/10">
              ...
              <div className="font-mono text-muted-foreground">
                {(block.startProb * 100).toFixed(1)}% → {(block.endProb * 100).toFixed(1)}%
              </div>
            </div>
          </motion.div>
        );
      ...
      {/* Posterior bar */}
      <div className="flex items-center gap-3 py-1.5 border-t border-gold/15 mt-2 pt-3">
        <div className="w-36 text-xs font-semibold text-gold">Posterior</div>
        <div className="flex-1 h-8 bg-muted/20 rounded-lg relative overflow-hidden border border-gold/15">
          <motion.div
            className="absolute left-0 top-0 h-full rounded-lg"
            style={{
